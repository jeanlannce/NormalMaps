...
namespace DynamicMaps.UI.Components
{
    ...
    public class MapLayer : MonoBehaviour
    {
        ...
        public static MapLayer Create(GameObject parent, string name, MapLayerDef def, float degreesRotation)
        {
            ...
            // v1.2.1 兼容：图层 sprite 统一走 SvgUtils（v1.2.1 数据图层全为 SVG），失败时图层跳过
            var sprite = SvgUtils.GetOrLoadCachedSprite(def);
            if (sprite == null)
            {
                Plugin.Log.LogWarning($"[MapLayer] Layer {name} sprite is null (svg render failed): {def.ImagePath}");
                return null;
            }
