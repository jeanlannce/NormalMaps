...
namespace DynamicMaps.UI.Components
{
    public class MapView : MonoBehaviour
    {
        ...
        public float ZoomMain { get; private set; }     // 主地图（Tab/peek）缩放
        public float ZoomMini { get; private set; }     // 小地图缩放
        public bool IsMiniMapActive { get; set; }       // 当前是否处于小地图模式

        /// <summary>当前生效的缩放（小地图模式用小地图缩放，否则用主地图缩放）</summary>
        public float ZoomCurrent => IsMiniMapActive ? ZoomMini : ZoomMain;

        /// <summary>主地图位置记忆（小地图模式下不动主地图视角）</summary>
        public Vector2 MainMapPos { get; private set; }
        ...
        public void LoadMap(MapDef mapDef)
        {
            ...
            Plugin.Log.LogInfo($"[MapView] Loading map {mapDef.DisplayName}: {mapDef.Layers.Count} layers");
            ...
            foreach (var pair in mapDef.Layers.OrderBy(pair => pair.Value.Level))
            {
                ...
                {
                    // 单图层失败不中断整张地图（SVG/PNG 渲染问题）
                    Plugin.Log.LogError($"Failed to load layer {layerName} ({layerDef.ImagePath}): {e.Message}");
                }
            }
        ...
        private void ApplyZoom(float zoom, float tweenTime)
        {
            ...
            {
                thing.GetRectTransform().DOScale(1 / zoom * Vector3.one, tweenTime);
            }
        }

        private void HandleZoomMainChanged(float normalized)
        {
            var zoom = NormalizedToActual(normalized);
            ZoomMain = zoom;
            if (!IsMiniMapActive)
            {
                ApplyZoom(zoom, 0f);
        ...
        public void ShiftMap(Vector2 shift, float tweenTime, bool isMini)
        {
            ...
            if (tweenTime == 0f)
            {
                // 直接赋值：每帧跟随等零时长场景避免创建 DOTween tween（减少 GC 分配）
                RectTransform.anchoredPosition = _immediateMapAnchor;
            }
            else
            {
